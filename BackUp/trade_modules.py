...
def get_next_expiry_date(e_day='thursday'):
    ...
    d1 = exp_day[e_day]
    today = datetime.now()
    holidays = json.load(open('config.json', 'r'))['HOLIDAY_LIST']
...
# GET PCR VALUES
def getPCR(symCode='NIFTY', spot_px = 20103):
    curr_dt = datetime.now().astimezone(pytz.timezone('Asia/Kolkata'))
    oi_pcr_dict = {}
    ...
    while i < len(data['filtered']['data']):
        ...
        if dct['CE']['changeinOpenInterest']>0:
            oi_change_pcr = round(abs(dct['PE']['changeinOpenInterest'])/abs(dct['CE']['changeinOpenInterest']),3)
        else:
            oi_change_pcr = 0
