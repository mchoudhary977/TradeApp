# -*- coding: utf-8 -*-
"""
ICICIDirect - Bollinger Bands Implementation
...
def bollBnd(DF,n,m):
    ...
    df["BB_up"] = df["MA"] + m*df['close'].rolling(n).std(ddof=0) #ddof=0 is required since we want to take the standard deviation of the population and not sample
    df["BB_dn"] = df["MA"] - m*df['close'].rolling(n).std(ddof=0) #ddof=0 is required since we want to take the standard deviation of the population and not sample
    df["BB_width"] = df["BB_up"] - df["BB_dn"]
    df.dropna(inplace=True)
    return df

ohlc = fetchOHLC("NIFTY","5minute",5)
bollBnd = bollBnd(ohlc,20,2)