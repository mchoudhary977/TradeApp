...
def get_data():
    ...
    oipcr = pd.read_csv(oi_pcr_file)
    unique_symbols = oipcr['CODE'].unique()
    oipcr_dict = {}
    for i in unique_symbols:
        oipcr_dict[i] = oipcr.loc[oipcr['CODE']==i].groupby('CODE').head(5)

    options_df = pd.read_csv(options_file)
    coipcr = pd.read_csv('COIPCR.csv')
    unique_symbols = options_df['SC'].unique()
    options_dict = {}
    ...
    for i in unique_symbols:
        ...
        print(spot_px)
        strike_step = 100 if i == 'CNXBAN' else (40 if i == 'NIFFIN' else 50)
        atm_strike = int(round(spot_px/50,0)*50) if i != 'CNXBAN' else int(round(spot_px/100,0)*100)
        strike_begin = atm_strike - (7*strike_step)
        strike_end = (7*strike_step) + atm_strike
        options_dict[i] = options_df.loc[options_df['SC']==i][options_df['STRIKE']>=strike_begin][options_df['STRIKE']<=strike_end]

    resultDict['OIPCR'] = oipcr_dict
    resultDict['COIPCR'] = options_dict
...
def get_trade_details():
    ...
    no_trades = 'No Trades'
    inactive_trades = pd.DataFrame(columns=[no_trades])
    failed_trades = pd.DataFrame(columns=[no_trades])
    closed_trades = pd.DataFrame(columns=[no_trades])
    active_trades = pd.DataFrame(columns=[no_trades])
    
    if len(trades) > 0:
        inactive_trades = trades[trades['Status'] == 'INACTIVE']
        failed_trades = trades[trades['Status'] == 'FAILED']
        closed_trades = trades[trades['Status'] == 'CLOSED']        
        active_trades = trades[trades['Status'] == 'TRADED']
        
        inactive_trades = inactive_trades if len(inactive_trades) > 0 else pd.DataFrame(columns=[no_trades])
        failed_trades = failed_trades if len(failed_trades) > 0 else pd.DataFrame(columns=[no_trades])
        closed_trades = closed_trades if len(closed_trades) > 0 else pd.DataFrame(columns=[no_trades])
        active_trades = active_trades if len(active_trades) > 0 else pd.DataFrame(columns=[no_trades])
        
    resultDict['InActive Trades'] = inactive_trades.to_html(index=False)
    resultDict['Failed Trades'] = failed_trades.to_html(index=False)
    resultDict['Closed Trades'] = closed_trades.to_html(index=False)
    resultDict['Active Trades'] = active_trades.to_html(index=False)
    
    return render_template('trades.html', tdate=datetime.now().strftime("%B %d, %Y %H:%M"),
                           resultDict=resultDict)
