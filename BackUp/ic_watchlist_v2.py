...
# icici.subscribe_feeds('4.1!85892')
# icici.unsubscribe_feeds('4.1!85892')
...
def check_option_list(symbol, price):
    # print('starting option check thread')
    global options_df
    # symbol = 'NIFTY 50'
    # underlying_px = 20101
    strike_range = 5 * int(json.load(open('config.json', 'r'))[symbol]['STRIKE_STEP'])
    ...
    # if '4.1!59241' in token_list:
    # options_df[options_df['PUT_TK']=='59241']
    list_to_subscribe = []
    list_to_unsubscribe = []
    for i in exist_tk:
    ...
    # print(f"updated token list - {updated_tk}")
   
    if len(list_to_subscribe) > 0:
        # print(f"List to subscribe - {list_to_subscribe}")
        ic_subscribeFeed(list_to_subscribe)
    if len(list_to_unsubscribe) > 0:
        # print(f"List to Unsubscribe - {list_to_unsubscribe}")
...
def on_ticks(ticks):
    ...
    if tick_time.second == 10 and (tick_symbol == 'NIFTY 50' or tick_symbol == 'NIFTY BANK' or tick_symbol == 'NIFTY FIN SERVICE'):
        option_check_thread = Thread(target=check_option_list,args=(tick_symbol,ticks['last']))
            ...
            if (now.time() >= time(9,14) and now.time() < time(15,35,0)):
                if subscription_flag=='N':
                    if os.path.exists(watchlist_file):
                        ...
                        wl_df = pd.read_csv(watchlist_file)
                        livePrices = wl_df
                        print('EEEEEEEEEEEENNNNNNNNNNNNNDDDDDDDD')
